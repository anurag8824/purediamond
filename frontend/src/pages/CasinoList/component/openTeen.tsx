...
const OpenTeen = (props: any) => {
    ...
    (data: string) => {
      ...
      const dataArray: Array<string> =
        finalString && finalString[0]
          ? finalString[0].split(",").filter((d) => d !== "1")
          : [];
  ...
  const laybacklayout = () => {
    ...
    return liveMatchData?.defaultMarkets?.map(
      (ItemMarketLink: any, keyLink: number) => {
        return ItemMarketLink?.Runners?.map((ItemNew: any, key: number) => {
          ...
          const clsstatus =
            ...
            Item.gstatus === "0"
              ...
              : "";
          const otherMarket =
            liveMatchData?.defaultMarkets?.[keyLink + 8]?.Runners[0] || {};
          const ItemOther: any = lastOdds?.[otherMarket.SelectionId] || {};
          const clsstatus2 =
            !ItemOther.gstatus ||
            ItemOther.gstatus === "SUSPENDED" ||
            ItemOther.gstatus === "CLOSED" ||
            ItemOther.gstatus === "0"
              ? "suspended"
              : "";
              console.log(keyLink ,"dfvvg")
          return (
            keyLink < 8 && (
              <tr key={key} className={` ${heightdata} ${clsstatus}`}>
                <td className={clsnamehead} style={{ paddingLeft: "10px" }}>
                  ...
                  {cards &&
                    cards[keyLink] &&
                    cards[keyLink]?.map((ItemCard: any, keyCard: any) => {
                      return (
                        <span
                          key={keyCard}
                          className={`card-icon ${
                            keyCard == 0 ? "m-l-20" : "m-l-1"
                          }`}
                          style={{
                            display: "inline-flex",
                            marginLeft:
                        ...
                        >
                          ...
                          {ItemCard.includes("CC") && (
                            <span className="card-black">]</span>
                          )}{" "}
                          {ItemCard.includes("HH") && (
                            <span className="card-black">{"}"}</span>
                          )}{" "}
                          {ItemCard.includes("DD") && (
                            <span className="card-red">{"{"}</span>
                          )}{" "}
                          {ItemCard.includes("SS") && (
                            <span className="card-red">[</span>
                          )}
                        </span>
                      );
                ...
                <td className={`back teen-section box-3 `}>
                  <button
                    className="back"
                    onClick={() => onBet(true, ItemOther)}
                  >
                    <span className="odd">
                      <b>{ItemOther.nation}</b>
                    </span>{" "}
                    <CasinoPnl
                      sectionId={ItemOther.sid}
                      matchId={liveMatchData.match_id}
                      classData={"text-center"}
                    />
                  </button>
            ...
            <table className="table coupon-table table table-bordered suspendwidth">
              <thead style={{ borderBottom: "0px" }}>
                <tr>
                  ...
                  <th className={`back ${"box-3"}`}>
                    Back (Min: {liveMatchData?.event_data?.min} Max:{" "}
                    {liveMatchData?.event_data?.max}){" "}
                  </th>
                  <th className={`back ${"box-3"}`}>
                    Min: {liveMatchData?.event_data?.min} Max:{" "}
                    {liveMatchData?.event_data?.max}
                  </th>
                </tr>
              </thead>
