...
{/* <tr>
  ...
  <td colSpan={sportListState.sports?.length}>
    <input
      ...
      min="0"
      max={Math.min(
        userData?.partnership?.[1]?.ownRatio || 100,
        userData?.partnership?.[2]?.ownRatio || 100,
        userData?.partnership?.[4]?.ownRatio || 100
      )}
      {...register("partnership.shared", {
        onChange: (e) => {
          const value = Number(e.target.value);

          // Set same value for all sportIds
          [1, 2, 4].forEach((sportId) => {
            //@ts-expect-error
            setValue(`partnership.${sportId}`, value);
            const ownRatio = userData.partnership?.[sportId]?.ownRatio;
            if (ownRatio !== undefined) {
              setValue(`partnershipOur.${sportId}`, ownRatio - value);
            }
          });
        },
      })}
